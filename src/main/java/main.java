...
public class main {
    ...
    public static void startBPMNconsole() throws MalformedURLException {
        ...
        switch (antwort) {
            case "1": {
                System.out.println("Bevor Sie Ihre Bestellung tätigen können, müssen Sie sich registrieren.");
    ...
    public static void startBPMNauto() throws MalformedURLException {
        ...
        System.out.println("Eingegeben: "+ stadt);
        System.out.println("Bitte geben Sie Ihre Straße an:");
        String straße = "Paul-Wittsack-Strasse 10";
        System.out.println("Eingegeben: "+ straße);
        System.out.println("Bitte geben Sie Ihre Postleitzahl  an:");
        String plz = "68199";
        System.out.println("Eingegeben: "+plz);
        System.out.println("Zum Schluss benötigen wir noch Ihre Telefonnummer:");
        String tel = "823892379";
        System.out.println("Eingegeben: "+tel);
        ...
        i++;
            ...
            switch (antwort2) {
                case "1": {
                    ...
                    System.out.println("Nachfolgend finden Sie die Liste aller verfügbaren Items:");
                    System.out.println("--------------------------------------------------");
                    String json2 = itemController.getAllItems();
                    ...
                    while (nochmal == true) {
                        ...
                        int bestand = Integer.parseInt(warehouseController.getItemStock(customerWID, itemid));
                        int array[] = new int[2];
                        System.out.println(anzahl + " Stück.");
                        if (bestand >= anzahl) {
                            System.out.println("Sie haben Glück!. Es sind genügend Items in ihrem Warehouse vorhanden. Der Versand kann sofort beginnen.");
                            JSONObject jsonObject = new JSONObject();
                            jsonObject.put("wid",customerWID);
                            ...
                            boolean found = false;
                            for (int v = 0; v < orderItemList.size(); v++){
                                int[] a = (int[]) orderItemList.get(i);
                                if (a[0] == itemid){
                                    a[1] += anzahl;
                                    orderItemList.set(i,a);
                            ...
                            boolean found = false;
                            for (int m = 0; m < orderItemList.size(); m++){
                                int[] a = (int[]) orderItemList.get(m);
                                if (a[0] == itemid){
                                    a[1] += anzahl;
                                    orderItemList.set(m,a);
                                    found = true;
                                }
                            }
                            if (found == false) {
                    ...
                    for (int z = 0; z < orderItemList.size(); z++) {
                        int[] array = (int[]) orderItemList.get(z);
                        System.out.println("ItemID: " + array[0] + " Menge: " + array[1]);
                    }
                    System.out.println("--------------------------------------------------");
                    System.out.println("Die Bestellung wird an folgende Adresse versendet: ");
                    ...
                    for (int u = 0; u < orderItemList.size(); u++) {
                        int[] array = (int[]) orderItemList.get(u);
                        orderREST.createOrderline(array[1], orderid, array[0]);
                    }
                    System.out.println("Orderlines erfolgreich gespeichert");
                    System.out.println("--------------------------------------------------");
                ...
                case "3": {
                    ...
                    System.out.println("--------------------------------------------------");
                    System.out.println("Bitte geben Sie die neue Straße ein");
                    String straßeneu = "NeueStraße";
                    System.out.println("Bitte geben Sie die neue Stadt ein");
                    String stadtneu = "NeueStadt";
                    System.out.println("Bitte geben Sie die neue Postleitzahl ein");
                    String plzneu = "10232";
                    System.out.println("Bitte geben Sie die neue Telefonnummer ein");
                    String telneu = "03289323";

                    JSONObject json3 = new JSONObject();
                    json3.put("c_TELEFONNUMMER", tel);
                    json3.put("c_STRASSE", straße);
                    json3.put("c_STADT", stadt);
                    json3.put("c_PLZ", plz);
                    customerController.updateCustomer(customerID, json3.toString());
                    System.out.println("--------------------------------------------------");
                    System.out.println("Adressdaten erfolgreich aktualisert!");
                    System.out.println("--------------------------------------------------");
                    break;
